...
//
// `progress` is updated at each phase milestone so the UI can show human-readable
// progress labels instead of just a spinner.
export const mappingJobsTable = pgTable("mapping_jobs", {
  ...
  progress:  text("progress"),             // human-readable current phase label
  result:    jsonb("result"),
  error:     text("error"),
