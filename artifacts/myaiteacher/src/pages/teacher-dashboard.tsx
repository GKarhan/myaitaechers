...
function LessonNodesPanel({
  ...
  practicalTasks?: { task: string; sourcePage?: string | null }[];
}) {
  const qc = useQueryClient();
        ...
        <div className="px-4 pb-4 space-y-3">
          {coreProblem && (
            <div>
              ...
              <p className="text-xs text-muted-foreground/80 leading-relaxed">{coreProblem}</p>
            </div>
          )}
          {coreIdea && (
            <div>
              <p className="text-xs font-semibold text-muted-foreground mb-0.5">💡 Գլխավոր գաղափար</p>
              <p className="text-xs text-muted-foreground/80 leading-relaxed">{coreIdea}</p>
            </div>
          )}
          ...
          {practicalTasks.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-muted-foreground mb-1">✏️ Վառժություններ</p>
              <div className="space-y-1">
