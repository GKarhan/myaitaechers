 ...
 * Uses lesson 105 (real mapped lesson — read-only verifications) plus
 * fresh temporary rows that are cleaned up in finally blocks.
 */
...
const LESSON_ID = 105; // real mapped lesson used throughout
const OTHER_LESSON_ID = 1; // for cross-lesson rejection tests (different lesson)

// ─── Helpers ─────────────────────────────────────────────────────────────────

/** Insert a temporary topic and return its id */
async function insertTempTopic(title: string, lessonId = LESSON_ID): Promise<number> {
  const [max] = await db
...
/** Insert a temporary node and return its id */
...
// Baseline: real lesson 105 topics
...
await test("topic ID from another lesson rejected", async () => {
  // Find a topic from a different lesson
  const otherTopics = await db
    .select({ id: lessonTopicsTable.id })
    .from(lessonTopicsTable)
    .where(eq(lessonTopicsTable.lessonId, OTHER_LESSON_ID))
    .limit(1);
  if (otherTopics.length === 0) { console.log("    (skipped: no topics in other lesson)"); return; }
  const alienId = otherTopics[0].id;
  await assert.rejects(
...
// We use temporary nodes so we don't disturb real lesson data
// Insert 4 temp nodes; remember original real nodes to restore later
const realNodes = await db
  .select({ id: lessonNodesTable.id, sequence: lessonNodesTable.sequence })
  .from(lessonNodesTable)
  .where(eq(lessonNodesTable.lessonId, LESSON_ID))
  .orderBy(asc(lessonNodesTable.sequence));
const realNodeIds = realNodes.map((n) => n.id);

// For isolated node-reorder tests, insert 4 temp nodes in a scratch lesson
// We use a sub-test approach: insert, test, delete
let tn1 = 0, tn2 = 0, tn3 = 0, tn4 = 0;
try {
  tn1 = await insertTempNode("TN1");
  tn2 = await insertTempNode("TN2");
  tn3 = await insertTempNode("TN3");
  tn4 = await insertTempNode("TN4");

  const allIds = [...realNodeIds, tn1, tn2, tn3, tn4];
  ...
  await test("node ID from another lesson rejected", async () => {
    const otherNodes = await db
      .select({ id: lessonNodesTable.id })
      .from(lessonNodesTable)
      .where(eq(lessonNodesTable.lessonId, OTHER_LESSON_ID))
      .limit(1);
    if (otherNodes.length === 0) { console.log("    (skipped: no nodes in other lesson)"); return; }
    const alienId = otherNodes[0].id;
    await assert.rejects(
  ...
  await test("REQUIRED deps preserved across reorder", async () => {
    ...
    try {
      ...
      await db.delete(lessonNodeDependenciesTable).where(eq(lessonNodeDependenciesTable.id, reqDep.id));
    }
  });

  await test("no self-edge produced", async () => {
    await reorderNodes(LESSON_ID, allIds);
    const selfEdges = await db
      .select({ id: lessonNodeDependenciesTable.id })
      .from(lessonNodeDependenciesTable)
      .where(and(
        eq(lessonNodeDependenciesTable.lessonId, LESSON_ID),
        // self-edge: fromNodeId = toNodeId — use raw SQL check
      ));
    // Filter manually (Drizzle doesn't expose column=column directly without sql``)
    // Sufficient to check all SEQUENTIAL edges have from ≠ to
    const seqDeps = await db
  ...
  for (const id of [tn4, tn3, tn2, tn1]) {
    if (id) await db.delete(lessonNodesTable).where(eq(lessonNodesTable.id, id)).catch(() => {});
  }
  // Restore original sequences for real nodes
  await db.transaction(async (tx) => {
    for (let i = 0; i < realNodeIds.length; i++) {
      await tx.update(lessonNodesTable).set({ sequence: i + 1 }).where(eq(lessonNodesTable.id, realNodeIds[i]));
    }
  }).catch(() => {});
...
let tempExId = 0;
try {
  tempNodeId = await insertTempNode("TEMP_DEL_NODE");
  // Insert exercise linked to the temp node
  const [exRow] = await db
    .insert(lessonExercisesTable)
    .values({
      lessonId: LESSON_ID,
      exerciseId: "EX-TEST-DEL",
      exerciseTextVerbatim: "TEMP_DEL_EXERCISE",
  ...
  if (tempNodeId) await db.delete(lessonNodesTable).where(eq(lessonNodesTable.id, tempNodeId)).catch(() => {});
  if (tempExId) await db.delete(lessonExercisesTable).where(eq(lessonExercisesTable.id, tempExId)).catch(() => {});
}
...
try {
  topicA = await insertTempTopic("TEMP_TOPIC_A");
  topicB = await insertTempTopic("TEMP_TOPIC_B");
  relNode = await insertTempNode("TEMP_TOPIC_NODE", LESSON_ID, topicA);
  ...
  await test("node topicId can be updated to a different topic", async () => {
    await db.update(lessonNodesTable).set({ topicId: topicB }).where(eq(lessonNodesTable.id, relNode));
    const [row] = await db
      .select({ topicId: lessonNodesTable.topicId })
      .from(lessonNodesTable)
      .where(eq(lessonNodesTable.id, relNode));
  ...
  await test("node topicId can be set to NULL (standalone)", async () => {
    await db.update(lessonNodesTable).set({ topicId: null }).where(eq(lessonNodesTable.id, relNode));
    const [row] = await db
      .select({ topicId: lessonNodesTable.topicId })
      .from(lessonNodesTable)
      .where(eq(lessonNodesTable.id, relNode));
  ...
  await test("topic delete sets node topicId to NULL (ON DELETE SET NULL)", async () => {
    ...
    await db.update(lessonNodesTable).set({ topicId: topicA }).where(eq(lessonNodesTable.id, relNode));
    await db.delete(lessonTopicsTable).where(eq(lessonTopicsTable.id, topicA));
  ...
  if (relNode) await db.delete(lessonNodesTable).where(eq(lessonNodesTable.id, relNode)).catch(() => {});
  if (topicB)  await db.delete(lessonTopicsTable).where(eq(lessonTopicsTable.id, topicB)).catch(() => {});
  if (topicA)  await db.delete(lessonTopicsTable).where(eq(lessonTopicsTable.id, topicA)).catch(() => {});
}
